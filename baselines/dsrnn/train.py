...
def main():
	...
	if config.training.resume: #retrieve the model if resume = True
		...
		if getattr(config.training, 'reinit_action_head', False):
			# Warm-start with a shape-aware load. Always drop the action head
			# (dist.*): its output semantics differ under the accel-limited tanh
			# scheme (pre-tanh delta-omega vs the source's raw delta-theta), so
			# those weights are not transferable. If reinit_trunk is set, ALSO
			# drop the policy/value trunks (base.actor.* / base.critic*) so a
			# warm-start whose trunk was shaped for a different action space
			# (e.g. HEIGHT's discrete-turtlebot checkpoint) can't bias the fresh
			# continuous-control policy — perception (edge RNNs, RH/HH attention)
			# still transfers. Load is shape-aware and strict=False, so anything
			# not matched (incl. the dropped prefixes) stays freshly initialized.
			drop_prefixes = ('dist.',)
			if getattr(config.training, 'reinit_trunk', False):
				drop_prefixes = drop_prefixes + ('base.actor.', 'base.critic.', 'base.critic_linear')
			model_sd = actor_critic.state_dict()
			ckpt = {k: v for k, v in ckpt.items()
					if not k.startswith(drop_prefixes)
					and k in model_sd and model_sd[k].shape == v.shape}
			missing, unexpected = actor_critic.load_state_dict(ckpt, strict=False)
			print("Warm-start from:", load_path)
			print("  reinit_trunk=%s -> dropped prefixes: %s" % (
				getattr(config.training, 'reinit_trunk', False), drop_prefixes))
			print("  loaded %d shape-matched tensors" % len(ckpt))
			print("  freshly-initialized params (missing):", missing)
		else:
	...
	# Track the best rolling mean episode reward so we can keep a best.pt.
	# PPO here is non-monotonic (a good policy can catastrophically collapse
	# late in training and not recover), so the LAST checkpoint is often not
	# the best one to use as the baseline — best.pt captures the peak.
	best_mean_reward = -float('inf')
	best_min_episodes = 50  # require a warmed-up window before trusting the mean
	...
	for j in range(num_updates):
		...
		if (j % config.training.save_interval == 0
			...
			# Best-so-far checkpoint (by rolling mean reward). Use best.pt as the
			# evaluation baseline rather than the final checkpoint.
			if len(episode_rewards) >= best_min_episodes:
				...
				if mean_r > best_mean_reward:
					...
					print("  new best.pt @ update %d: mean reward %.2f" % (j, mean_r))
