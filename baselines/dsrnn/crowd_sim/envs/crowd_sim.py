...
class CrowdSim(gym.Env):
    ...
    def configure(self, config):
        ...
        if self.config.humans.policy == 'orca' or self.config.humans.policy == 'social_force':
            ...
            # observed_human_num = the FIXED network input size (the SRNN's
            # spatial-edge graph is built for exactly this many slots).
            # self.human_num is the ACTUAL number of humans spawned, which for
            # the 'evaluation' scenario now VARIES per episode in
            # [min_human_num, max_human_num] (drawn in reset()) to match
            # crowdnav_env's eval (randint(1,11) -> 1..10). generate_ob
            # feeds the network the closest observed_human_num humans + padding,
            # IDENTICAL to the eval adapters (crowd_nav/policy/dsrnn.py and
            # height.py predict()). Collision/reward/human-actions still use all
            # actual humans (self.humans).
            self.observed_human_num = config.sim.human_num
            self.min_human_num = getattr(config.sim, 'min_human_num', config.sim.human_num)
            self.max_human_num = getattr(config.sim, 'max_human_num', config.sim.human_num)
            # Padding offset for empty spatial-edge slots. MUST equal the eval
            # adapters' _pad_offset (2*square_width) so DSRNN — which has no
            # attention masking — sees the exact same padding at train and eval.
            self.pad_offset = 2.0 * config.sim.square_width
    ...
    # Additive scenario matching crowdnav_env's env.config
    # "evaluation" mode: robot start/goal random inside a square (goal >5m
    # from start), humans randomly placed in the same square (rejecting
    # spawns that collide with the robot or already-placed humans, same
    # convention used by generate_circle_static_obstacle above). Only used
    # when config.sim.scenario == 'evaluation'; DSRNN's own circle_crossing
    # generator (generate_robot_humans below) is untouched otherwise.
    def _generate_evaluation_robot_humans(self, human_num):
        half_w = self.config.sim.square_width / 2

        # Robot: RANDOM start and goal inside the square box (goal >5 m from
        # start), random heading — IDENTICAL to crowdnav_env's eval
        # 'evaluation' branch (crowd_sim.py: px_r/py_r/gx_r/gy_r ~
        # U(-square_width/2, square_width/2), heading ~ U(-pi, pi)). The box
        # extent (square_width), not a circle, defines the scenario — there is
        # no fixed (0,-r)->(0,+r) robot task here.
    ...
    def generate_robot_humans(self, phase, human_num=None):
        ...
        if getattr(self.config.sim, 'scenario', 'circle_crossing') == 'evaluation':
            self._generate_evaluation_robot_humans(human_num)
            return
    ...
    def calc_reward(self, action):
        ...
        if self.robot.kinematics == 'unicycle':
            ...
            #
            # The -2 above was calibrated by DSRNN's own authors for their
            # original unicycle example model, whose raw per-step delta-theta
            # was hard-clipped to +-0.1 rad with no rate-limit dynamics (see
            # data/example_model_unicycle, commit 3eb7116). This project's
            # common kinodynamic limits (held common across all baselines)
            # let action.r reach max_wrot*time_step rad/step instead, which
            # is several times larger — squared in a quadratic penalty, that
            # made turning cost far more than DSRNN's authors intended
            # relative to the max achievable forward-progress reward, and
            # empirically stalled training (reward pinned around -75..-90
            # instead of climbing like their published holonomic curve).
            # Rescaled here so the penalty at our actual max turn rate has
            # the same relative severity their -2 had at their own reference
            # ceiling, rather than changing the reward function's intent.
            _dsrnn_ref_max_r = 0.1
            _our_max_r = self.config.action_space.max_wrot * self.time_step
            _rspin_scale = (_dsrnn_ref_max_r / _our_max_r) ** 2 if _our_max_r > 0 else 1.0
            r_spin = -2 * _rspin_scale * action.r**2
            ...
            # Small additive reward for forward speed itself — not part of
            # DSRNN's own published reward, added specifically to counteract
            # a cold-start trap that only exists because of the common
            # accel-rate-limited accumulator (self.desiredVelocity in
            # crowd_sim_dict.py step()), not because of anything in DSRNN's
            # own reward design. DSRNN's original action space (both
            # holonomic and their own unicycle example) applies the raw
            # per-step output directly as velocity, so a network can reach
            # v_pref in a single step; ours ramps from v=0 over several
            # steps (max_dv=max_accel*time_step per step), so a policy has
            # to commit to multiple consecutive positive-dv steps before the
            # existing potential-based term above pays off at all. Since
            # standing still costs nothing (no collision risk, zero
            # potential-based delta) that cold start is a stable, easily
            # discovered equilibrium — confirmed empirically: with this
            # term absent, both DSRNN's and HEIGHT's own retraining runs
            # converged to a policy with v pinned at exactly 0.0 for the
            # entire episode, and reward flat for the full run. Scaled
            # small (max 0.1*v_pref=0.1/step) relative to collision
            # (-20) and turn (~-0.02/step at max, see r_spin above) so it
            # nudges the policy to actually try moving without overriding
            # collision avoidance or the terminal success/collision terms
            # that are the core of DSRNN's own reward design.
            r_move = 0.1 * max(action.v, 0.0)

            # print(reward, r_spin, r_back, r_move)
            reward = reward + r_spin + r_back + r_move
