...
class CollisionAvoidMPC(Policy):
    def __init__(self):
        ...
        # Staleness tolerance used by bring_fwd(): if the human positions predicted by the
        # previous solution differ from the newly observed ones by more than this, the
        # forwarded solution is discarded and a fresh feasible warmstart is computed.
        #
        # Upstream read mpc_env.rob_rad_buffer here, overloading one config key for two
        # semantically unrelated things -- the collision-constraint radius and this
        # prediction-staleness threshold. Decoupled so that tuning the collision radius for
        # cross-baseline clearance parity (see rob_rad_buffer in sicnav/configs/policy.config)
        # cannot silently change how readily a stale forwarded solution is reused. That
        # matters here specifically because this eval case's pathology IS systematically-wrong
        # human prediction -- the robot is invisible to humans (env.config [robot]
        # visible=false), so they never reciprocate as the ORCA-KKT model assumes, and a
        # looser tolerance would suppress exactly the re-warmstarts that catch it.
        # Pinned to 0.15, the value in force before the clearance re-tune. Numerically this
        # is near-inert at the current buffer of 0.14; it is insurance, not a behaviour change.
        self.bring_fwd_pos_tol = 0.15

    # Overwrite set_env to also configure the policy's env-related values
    def set_env(self, env):
        super().set_env(env)
        # set_human_observability toggles CrowdSimPlus's own human-policy visibility of the
        # robot; it has no equivalent on host envs this policy is adapted onto (e.g.
        # crowdnav_env's CrowdSim), where privileged vs. non-privileged access is
        # instead handled entirely in predict() below. Skip when unsupported.
        if hasattr(env, 'set_human_observability'):
    ...
    def init_mpc(self, state, ipopt_print_level=0):
        # Must always resync to the current episode's human count, not just on the very
        # first call -- predict() calls init_mpc() again at the start of every episode
        # (self.env.global_time == 0.0), and human count varies per episode under e.g.
        # crowdnav_env's 'evaluation' scenario (human_num = randint(1, 11) each
        # reset). Reusing a stale self.num_hums here desyncs the MPC's sizing (agent_radii,
        # constraint counts, etc., all built from self.num_hums) from the actual state
        # (built from len(state.human_states)), causing an IndexError deep in
        # orca_casadi.py the moment the human count changes between episodes.
    ...
    def _warmstart_om_rate_ok(self, u_guess):
        """Does the ORCA-based warmstart respect the omega-RATE limit?

        DELIBERATELY checks omega-rate ONLY, not v's bounds or acceleration.
        A variant that also gated on v was measured over 500 paired episodes and
        was SIGNIFICANTLY WORSE: 4.40% collisions vs 2.60% (McNemar p = 0.049,
        13 episodes lost / 4 gained). Reason: warmstart v-infeasibility is
        numerically negligible (measured < 1e-4, i.e. micrometres of travel), so
        gating on it rejects useful warmstarts for no safety benefit and forces
        campc to keep an IPOPT solution its own test just judged worse. The v
        limits are instead enforced by a hard clamp in the adapter
        (crowd_nav/policy/sicnav.py), which costs nothing. Omega-rate is
        different: violations there were large and caused real collisions.

        The warmstart is produced by a SEPARATE CasADi function
        (get_rob_warmstart_fn), not by the opti problem, so it does NOT carry the
        omega-rate constraint added in get_opti_dict. select_action discards the
        optimised solution whenever the warmstart's objective is better -- and
        since the added constraint makes the optimised objective worse by
        construction, that override started firing ~20% of the time (up from ~4%
        upstream), letting infeasible turn rates straight through to the robot.
        Measured: it accounted for essentially all of the residual post-hoc
        clipping after the constraint was added.

        So: only allow the override when the warmstart is itself rate-feasible.
        A suboptimal-but-feasible plan beats a lower-cost plan the robot cannot
        physically execute. Returns True (upstream behaviour) when no limit is set.
        """
        if not np.isfinite(getattr(self.mpc_env, 'max_w_accel', np.inf)):
            return True
        try:
            lim = self.mpc_env.max_w_accel * self.mpc_env.time_step + 1e-6
            om = np.asarray(u_guess)[1, :].ravel()
            prev = float(getattr(self, 'prev_avel', 0.0))
            dom = np.diff(np.concatenate([[prev], om]))
            return bool(np.all(np.abs(dom) <= lim))
        except Exception as e:
            logging.warning('[CAMPC] omega-rate check on warmstart failed (%s); '
                            'keeping optimised solution', e)
            return False
    ...
    def get_opti_dict(self, K, K_orca=None, ipopt_print_level=0, do_spec=False):
        ...
        # ------------------------------------------------------------------
        # Angular-acceleration (omega-rate) limit.  [ADDED - not upstream]
        #
        # Upstream SICNav has no omega-rate limit anywhere: the robot state
        # X_r = (x, y, theta, v_prev) carries no omega term, and the cost has
        # ZERO penalty on omega changing between steps (the [mpc_env] COST
        # PARAMS block is dead config on THIS code path -- only
        # sicnav_diffusion/.../mpc_env_new.py reads it; the cost actually used
        # is hardcoded in mpc_env.py as R = diag([0, 0.1])). So the MPC plans
        # bang-bang angular motion that saturates its own +-max_rot bound.
        #
        # The host harness (crowdnav_env) enforces a per-step
        # |domega| <= max_w_accel*dt limit on EVERY policy. Without this
        # constraint the adapter had to clip the MPC's command after the fact,
        # measured at 34% of steps (vs 10% for the ORCA baseline, whose omega
        # comes from a smooth proportional heading controller). That made
        # SICNav the only policy here whose committed multi-step plan was
        # routinely invalidated before execution -- a handicap created by the
        # harness, not by SICNav.
        #
        # This is a constraint coupling ADJACENT COLUMNS of the input decision
        # matrix, so it needs no new state dimension. (An earlier assessment
        # assumed omega_prev had to become a 5th element of X_r, which would
        # have reindexed the entire bilevel ORCA-KKT formulation. It does not.)
        #
        # Kept HARD rather than slack-penalised because it cannot by itself
        # make the problem infeasible: omega ranges over +-max_rot and the
        # per-step budget is max_w_accel*dt, so the feasible interval for every
        # column is non-empty for any om_prev. om_prev is a PARAMETER holding
        # the previously EXECUTED omega (set by the adapter each step), not the
        # previously commanded one, so the constraint is anchored to the robot's
        # real angular state.
        # ------------------------------------------------------------------
        om_prev_param = opti.parameter(1, 1)
        if np.isfinite(self.mpc_env.max_w_accel):
            max_domega = self.mpc_env.max_w_accel * self.mpc_env.time_step
            om_row = u_var[1, :]
            domega = cs.horzcat(om_row[0, 0] - om_prev_param, om_row[0, 1:] - om_row[0, :-1])
            logging.info('[CAMPC] Adding omega-rate constraint |domega| <= {:.4f} rad/s over horizon'.format(max_domega))
            self.subject_to_const(opti, domega <= max_domega, k=-1, name_stub='rob_om_rate_ub')
            self.subject_to_const(opti, -domega <= max_domega, k=-1, name_stub='rob_om_rate_lb')
        else:
            logging.info('[CAMPC] max_w_accel not set/infinite -- omega-rate constraint DISABLED')
    ...
    def bring_fwd(self, joint_state, obs, x_prev, u_prev, for_guess=True):
        ...
        if self.mpc_env.hum_model == 'orca_casadi_kkt' and self.warmstart:
            ...
            if np.any(np.abs(x_prev_fwded[:3, 0] - obs[:3, 0]) > 1e-2) or np.any(np.abs(np.take(x_prev_fwded[:, 0], slice_indices_posns, axis=0) - np.take(obs[:, 0], slice_indices_posns, axis=0)) > self.bring_fwd_pos_tol):
    ...
    def select_action(self, obs, joint_state, goal_states, goal_actions, return_all=False, dummy=False):
        ...
        # Previously EXECUTED omega, anchoring the omega-rate constraint above.
        # self.prev_avel was vestigial upstream (reset but never assigned); the
        # adapter now writes the executed omega into it before each predict().
        opti.set_value(opti_dict["om_prev_param"], getattr(self, 'prev_avel', 0.0))
        ...
        try:
            ...
            if self.warmstart and final_obj_val > init_obj_val and self._warmstart_om_rate_ok(u_guess):
    ...
    def get_int_goal(self, state):
        ...
        if (getattr(self.env, 'sim_env', None) in ("hallway_static", "hallway_static_with_back", "hallway_bottleneck")) and np.min(ys) < self.env.door_y_mid_min and np.max(ys) > self.env.door_y_mid_max:
    ...
    def get_ref_traj(self, state):
        ...
        """
        if self.new_ref_each_step or (self.ref_type == 'point_stab' and (getattr(self.env, 'sim_env', None) in ("hallway_static", "hallway_static_with_back", "hallway_bottleneck"))):
