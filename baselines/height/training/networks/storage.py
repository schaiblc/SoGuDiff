...
class RolloutStorage(object):
    '''
    method: if ours, the only key of recurrent_hidden_states will be 'rnn'
            else (baseline), the keys will be 'human_node_rnn' and 'human_human_edge_rnn'
    '''
    def __init__(self, num_steps, num_processes, obs_shape, action_space,
                 human_node_rnn_size, method='ours'):
        ...
        if isinstance(obs_shape, dict):
            ...
            if 'spatial_edges' in obs_shape:
                if 'obstacle_vertices' in obs_shape:
                    self.human_num = obs_shape['spatial_edges'].shape[0] + obs_shape['obstacle_vertices'].shape[0]
                else:
                    self.human_num = obs_shape['spatial_edges'].shape[0]
            else: # a workaround for env that does not have spatial_edges
                self.human_num = 1
        else:
        ...
        if method == 'ours':
            self.recurrent_hidden_states['rnn'] = torch.zeros(num_steps + 1, num_processes, 1, human_node_rnn_size)
        else:
            node_num = 1
            ...
            self.recurrent_hidden_states['human_human_edge_rnn'] = torch.zeros(num_steps + 1, num_processes, edge_num, human_node_rnn_size)
