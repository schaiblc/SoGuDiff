...
class Config(object):
    ...
    # environment settings
    sim = BaseConfig()
    sim.circle_radius = 9 # radius of the circle where all humans start on (UNUSED by 'evaluation')
    sim.human_num = 10 # MAX crowd size = fixed observation slot count (STAR weights are sized human_num+1)
    sim.static_human_num = 0 # number of static humans
    sim.group_human = False
    # baselines/navistar: 'evaluation' = crowdnav_env's eval
    # scenario — robot start/goal uniform in a square (goal >5m from start),
    # ACTUAL crowd size drawn uniformly from 1..sim.human_num per episode;
    # unused observation slots are padded at (15,15) and masked out. The
    # author's circle_crossing generator is untouched otherwise.
    sim.scenario = 'evaluation'
    # matches crowdnav_env's env.config sim.square_width (square spans +/- square_width/2)
    sim.square_width = 15
    ...
    humans.policy = "orca"
    humans.radius = 0.25 # radius of each human — eval task value (was 0.3)
    humans.v_pref = 1 # max velocity of each human
    # FOV = this values * PI
    humans.FOV = 2.

    # a human may change its goal before it reaches its old goal
    humans.random_goal_changing = False # eval task: goals are fixed for the episode (was True)
    ...
    robot.radius = 0.25  # radius of the robot — eval task value (was 0.3)
    ...
    # baselines/navistar: was unconditionally True. With self-chaining resubmission
    # (see train_baselines/navistar.slurm) every restart is a resume-in-place, not
    # a fresh run — True would wipe data/navigation/star_sac/checkpoints/
    # (including training_state.pt) on every resubmit. Set
    # NAVISTAR_FRESH_START=1 for a deliberate from-scratch restart.
    training.overwrite = os.environ.get('NAVISTAR_FRESH_START', '0') == '1'
