...
class CrowdSim(gym.Env):
    ...
    # baselines/navistar, additive scenario matching crowdnav_env's
    # env.config "evaluation" mode — same generator as baselines/height /
    # baselines/dsrnn. Robot start/goal random inside a square (goal >5m
    # from start, random heading), humans randomly placed in the same square
    # (rejecting spawns that collide with the robot, already-placed humans, or
    # their goals). Only used when config.sim.scenario == 'evaluation'; the
    # author's circle_crossing generators are untouched otherwise.
    ...
    def generate_robot_humans(self, phase, human_num=None):
        ...
        # baselines/navistar 'evaluation' scenario: draw the ACTUAL crowd size
        # uniformly from 1..sim.human_num per episode. sim.human_num keeps its
        # configured value ONLY as the fixed observation slot count (STAR's
        # weights/positional embeddings are sized human_num+1); generate_ob
        # pads unused slots so every observation still matches the declared
        # observation_space.
        if getattr(self.config.sim, 'scenario', 'circle_crossing') == 'evaluation':
            self.human_num = int(np.random.randint(1, self.config.sim.human_num + 1))
    ...
    def generate_ob(self, reset):
        ...
        # baselines/navistar ('evaluation' scenario): the actual crowd varies
        # 1..sim.human_num across episodes, but every observation must keep
        # the DECLARED fixed sizes (SAC's replay buffer and STAR's positional
        # embeddings are sized human_num+1). Unused slots are parked at
        # (15, 15): outside visible_dis, so visible_masks zeroes them and the
        # attention mask treats them as padding — same convention as
        # baselines/height / baselines/dsrnn (inf->15 padding).
        n_slots = self.observation_space.spaces['spatial_edges'].shape[0]
        n_pad = n_slots - self.human_num
        ...
        if n_pad > 0:
            spatial_edges = np.concatenate((spatial_edges, np.full((n_pad, 2), 15.)), axis=0)
        ...
        one_hot = np.array([0, 1] * n_slots).reshape(n_slots, -1)
