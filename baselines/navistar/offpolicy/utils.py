...
def make_env(config):
    ...
    # baselines/navistar: gym 0.21's make() wraps the env in OrderEnforcing, and
    # gym.Wrapper.__setattr__ does NOT proxy plain attribute assignment to the
    # underlying env (only reads fall back via __getattr__) — so these must be
    # written via env.unwrapped or reset() never sees them (same gotcha worked
    # around in baselines/dsrnn / baselines/height).
    envSeed = config.env.seed if config.env.seed is not None else None
    env.unwrapped.thisSeed = envSeed
    env.unwrapped.nenv = 1
