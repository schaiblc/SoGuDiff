...
class GCN(nn.Module):
    ...
    def process_graph(graph):
        ...
        :return: [B * T, N, N]
        '''
        # Batched normalisation, replacing upstream's per-sample loop. The
        # loop issued ~5 small CUDA kernels per sample, so at SAC's
        # batch_size=1024 it launched ~5k kernels per update and dominated
        # step time (~0.8 s/step against 1.3 ms of environment stepping).
        # This computes the same \hat(A) = D^{-1} (A + I) for every sample in
        # a handful of kernels, elementwise-equal to the loop version.
        #
        # Unlike upstream this does not modify `graph` in place: the original
        # `g += eye` mutated the caller's tensor through the graph[i] view.
        # Callers build a fresh adjacency on every forward pass (see
        # get_adj_matrix), so nothing relied on that mutation.
        N = graph.shape[1]
        eye = torch.eye(N, dtype=graph.dtype, device=graph.device)
        g = graph + eye  # A~ [B*T, N, N]

        degree_matrix = g.sum(dim=-1)  # [B*T, N]
        degree_matrix = degree_matrix.pow(-1)
        degree_matrix[degree_matrix == float("inf")] = 0.  # [B*T, N]

        return torch.bmm(torch.diag_embed(degree_matrix), g)  # D^(-1) * A~
        return graph
