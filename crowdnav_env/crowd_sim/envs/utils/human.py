...
from crowd_sim.envs.utils.state import FullState, JointState, FullyObservableJointState
...
class Human(Agent):
    ...
    def get_g_xy(self, px, py):
        ...
        :return:
        """
        if self.env is not None and hasattr(self.env, "static_obstacles") and self.env.static_obstacles and len(self.env.static_obstacles) > 0:
            # xs = [px, self.final_gx]
            ys = [py, self.final_gy]
