...
class Agent(object):
    ...
    def step(self, action):
        ...
        if self.kinematics == 'holonomic':
            self.vx = action.vx
            self.vy = action.vy
            new_theta = np.arctan2(self.vy, self.vx)
            if self.theta is not None:
                d = (new_theta - self.theta + np.pi) % (2 * np.pi) - np.pi
                omega = d / self.time_step if self.time_step else 0.0
                # Clip to the policy's omega range so IL distribution doesn't blow up the net
                if hasattr(self, 'policy') and self.policy is not None and getattr(self.policy, 'max_wrot', None):
                    omega = float(np.clip(omega, -self.policy.max_wrot, self.policy.max_wrot))
                self.omega = omega
            else:
                self.omega = 0.0
            ...
            self.omega = action.r / self.time_step if self.time_step else 0.0
