...
class SFM(Policy):
    def __init__(self):
        ...
        # Planning-margin added on top of the true (self + other) collision
        # radii in the human-interaction force below — mirrors ORCA's
        # `+ safety_space` buffer. Defaults to 0 so plain SFM (used for
        # simulated humans via env.config's [humans] policy = orca/sfm) is
        # unaffected; SFMUnicycle overrides this to give the robot baseline
        # a real margin instead of planning right up against the collision
        # boundary.
    ...
    def predict(self, state):
        ...
        """
        vx, vy = self._compute_desired_velocity(state)
        return ActionXY(vx, vy)

    def _compute_desired_velocity(self, state):
        """
        Compute the SFM-desired holonomic velocity (vx, vy), clipped to
        v_pref. Factored out of predict() so SFMUnicycle can reuse the exact
        same force model and project the result onto unicycle (v, omega)
        instead of returning it directly as ActionXY.
        """
        # Pull force to goal
        ...
        for other_human_state in state.human_states:
            ...
            interaction_vx += A * np.exp((state.self_state.radius + other_human_state.radius + adjustment + self.safety_space - dist_to_human) / B) * (delta_x / dist_to_human)
            interaction_vy += A * np.exp((state.self_state.radius + other_human_state.radius + adjustment + self.safety_space - dist_to_human) / B) * (delta_y / dist_to_human)
        ...
        if act_norm > state.self_state.v_pref:
            return new_vx / act_norm * state.self_state.v_pref, new_vy / act_norm * state.self_state.v_pref
        else:
